...
export default class App extends Component {
  ...
  // Funtion to concat user input to output screen
  ...
  // Function to replace last index of the output
  _replaceLastIndex = (value) => {
    var str1 = this.state._output.replace(/.$/,value);
    this.setState({_output: str1});
  } 
  ...
  _evaluate = () => {
    try {
      ...
      if (isNaN(strCurOutput)) {
        ...
        this.setState({ _output: '' + dEval, _history: aHistory })
      }
    } catch (exception) {
  ...
  //Funtion to initalize output state
  _initOutput = () => {
  ...
  _clearHistory = () => {
    ...
    const emptyArray = [];
    this.setState({_history: emptyArray});
  }
